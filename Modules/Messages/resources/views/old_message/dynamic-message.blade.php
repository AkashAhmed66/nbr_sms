<form action="{{ route('messages.store-group-message') }}" method="POST">
  ...
  <div class="row g-6">
    ...
    <div class="form-floating form-floating-outline mb-6">
      <div class="form-floating form-floating-outline">
        <select id="group_ids" name="group_ids[]" class="select2 form-select form-select-lg" multiple="multiple" data-allow-clear="true">
          @foreach($groups as $key=>$group)
            <option value="{{$group->id}}">{{$group->name}}</option>
          @endforeach
        </select>
        <label for="formtabs-country">Select Group</label>
      </div>
    </div>
