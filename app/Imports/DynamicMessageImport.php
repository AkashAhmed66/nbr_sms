...
class DynamicMessageImport implements OnEachRow, WithChunkReading, WithHeadingRow, ShouldQueue
{
  ...
  public function __construct($messagePayload, int $userId = null, $dnd = null, $dndnumbers = [], $excelHeaders = [])
  {
    $this->messageResponse = $messagePayload;
  ...
  public function onRow(Row $row) {
    ...
    if (
      ...
      if(!$this->dnd || !in_array($phone, $this->dndnumbers)) {
        ...
        $microseconds = intval($mt[1] * 1E6) + intval(round($mt[0] * 1E6));

        //save to send message table
        $messageId = Message::create([
          'senderID' => $this->messageResponse['senderID'],
          'message' => $dynamicMessage,
          'sms_count' => $this->messageResponse['sms_count'],
          'source' => 'WEB',
          'date' => date('Y-m-d H:i:s'),
          'scheduleDateTime' => $this->messageResponse['scheduleDateTime'] ?? null,
          'Queue' => 'Queue',
          'content_type' => $this->messageResponse['content_type'],
          'campaign_name' => $this->messageResponse['campaign_name'] ?? 'WEB',
          'campaign_id' => $this->messageResponse['campaign_id'],
          'orderid' => $this->user->id . $microseconds,
          'user_id' => $this->messageResponse['user_id'] ?? $this->user->id,
          'IP' => request()->ip(),
          'is_masking' => $this->messageResponse['is_masking'],
          'total_recipients' => 1,
          'is_dnd_applicable' => $this->messageResponse['is_dnd_applicable'] ?? null,
          'is_unicode' => $this->messageResponse['is_unicode'],
     ...
     if (is_null($this->messageResponse['scheduleDateTime'])) {
        if (env('APP_TYPE') == 'Aggregator') {
          ...
          // } else {
            if($this->messageResponse['senderID'] == '1000000000000000000'){
              Log::channel('sms')->info("Sending to Smsc Api");
              $this->sendToSmscApi($outboxMessage);
            } else {
              Log::channel('sms')->info("Sending to Infozillion");
              $this->sendMessageToInfozilion($numbers, $messageId->id, $this->messageResponse['senderID']);
            }
          // }
        }
      }
