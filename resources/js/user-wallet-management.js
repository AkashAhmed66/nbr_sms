...
$(function () {
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    $.get(`${baseUrl}transactions/uwallet\/${user_id}\/edit`, function (data) {
      ...
      $('#add-user-name').val(jsonData.user_name);
  ...
  const addNewUserWalletForm = document.getElementById('addNewUserWalletForm');

  // user form validation
  const fv = FormValidation.formValidation(addNewUserWalletForm, {
    fields: {
      user_name: {
        validators: {
          notEmpty: {
    ...
    $.ajax({
      data: $('#addNewUserWalletForm').serialize(),
      url: url,
      type: method,
      success: function (response) {
