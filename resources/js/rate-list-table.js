...
$(function () {
  ...
  Object.entries(tableHeaders).forEach(function([key, value], index) {
    var columnDef = {
      ...
      sortable: true,
      render: function(data, type, full, meta) {
        var cellData = full[key];
        
        // Check if the data is a number and format to 2 decimal places
        if (typeof cellData === 'number' && !Number.isInteger(cellData)) {
          cellData = parseFloat(cellData).toFixed(2);
        } else if (typeof cellData === 'string' && !isNaN(cellData) && cellData.includes('.')) {
          cellData = parseFloat(cellData).toFixed(2);
        }
  ...
  if (data_table.length) {
    var dt_user = data_table.DataTable({
     ...
     columns: [ {data: ''}, Object.entries(tableHeaders).map(([key, value]) => {
      ...
      buttons: (userGroup === 1 || userGroup === 2) ? [ // Add button only if user group is 1
        {
