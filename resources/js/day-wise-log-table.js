...
$(function () {
  ...
  if (data_table.length) {
    var dt_user = data_table.DataTable({
      ...
      order: function () {
        // Find the column index for message_count
        var messageCountIndex = -1;
        Object.entries(tableHeaders).forEach(function ([key, value], index) {
          if (key === 'message_count') {
            messageCountIndex = index + 1; // +1 because first column is serial number
          }
        });
        // If message_count column found, sort by it descending, otherwise use default
        return messageCountIndex !== -1 ? [[messageCountIndex, 'desc']] : [[2, 'desc']];
      }(),
      dom:
