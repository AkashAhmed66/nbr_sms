...
$(function () {
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}phonebook/group-delete/${user_id}`,
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    // get data
    $.get(`${baseUrl}phonebook/group\/${user_id}\/edit`, function (data) {
      ...
      $('#add-type').val(jsonData.type).trigger('change');
      $('#add-reseller').val(jsonData.reseller_id).trigger('change');
  ...
  const addNewPhonebookGroupForm = document.getElementById('addNewPhonebookGroupForm');

  // user form validation
  const fv = FormValidation.formValidation(addNewPhonebookGroupForm, {
    fields: {
      name: {
        validators: {
          notEmpty: {
      ...
      reseller_id: {
        validators: {
          notEmpty: {
            message: 'Please select reseller'
          }
        }
      },
      status: {
    ...
    var url = isEditMode ? `${baseUrl}phonebook/group-update/${userId}` : `${baseUrl}phonebook/group-store`;
    var method = isEditMode ? 'PUT' : 'POST';
    //console.log($('#addNewOperatorForm').serialize());
    // adding or updating user when form successfully validate
    $.ajax({
      data: $('#addNewPhonebookGroupForm').serialize(),
      url: url,
      type: method,
      success: function (response) {
        ...
        Swal.fire({
          ...
          text: `Group ${response.status} Successfully.`,
          customClass: {
            confirmButton: 'btn btn-success'
          }
        }).then(() => {
