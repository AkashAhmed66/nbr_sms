...
$(function () {
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}phonebook/contacts-delete/${user_id}`,
          success: function (response) {
            window.location.href = `${baseUrl}phonebook/contacts-list`;
            dt_user.draw();
          },
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    // get data
    $.get(`${baseUrl}phonebook/contacts\/${user_id}\/edit`, function (data) {
      ...
      $('#add-group').val(jsonData.group_id).trigger('change');
  ...
  const addNewPhonebookContactForm = document.getElementById('addNewPhonebookContactForm');

  // user form validation
  const fv = FormValidation.formValidation(addNewPhonebookContactForm, {
    fields: {
      group_id: {
        validators: {
          notEmpty: {
    ...
    var url = isEditMode ? `${baseUrl}phonebook/contacts-update/${userId}` : `${baseUrl}phonebook/contacts-store`;
    var method = isEditMode ? 'PUT' : 'POST';
    //console.log($('#addNewOperatorForm').serialize());
    // adding or updating user when form successfully validate
    $.ajax({
      data: $('#addNewPhonebookContactForm').serialize(),
      url: url,
      type: method,
      success: function (response) {
        ...
        Swal.fire({
          ...
          text: `Contact ${response.status} Successfully.`,
          customClass: {
            confirmButton: 'btn btn-success'
          }
        }).then(() => {
