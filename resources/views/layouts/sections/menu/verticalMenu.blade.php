...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
  ...
  <div class="app-brand demo">
    <a href="{{url('/')}}" class="app-brand-link">
      <span class="app-brand-text demo menu-text text-center fw-semibold ms-2" style="margin-left: 3.5rem !important;">{{ config('variables.templateName') }}</span>
    </a>
  ...
  @foreach ($menuData[0]->menu as $menuItem)
      ...
      <li class="menu-item {{ $activeClass }}">
        ...
        <ul class="menu-sub">
          @foreach ($menuItem->submenu as $subItem)
          <li class="menu-item {{ $currentRouteName === $subItem->slug ? 'active' : '' }}">
            <a href="{{ url($subItem->url) }}" class="menu-link">
