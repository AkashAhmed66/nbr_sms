...
// Ensure that monthlyStatusCounts is available
  if (typeof monthlyStatusCounts !== 'undefined' && monthlyStatusCounts !== null) {
    // Prepare the series data for the chart
    const statusList = ['Failed', 'Delivered', 'Sent', 'Processing', 'Queue', 'Hold'];

    // Define custom colors for each status
    const statusColors = {
      'Failed': '#FF0000',    // Red for Failed
      'Delivered': '#28a745', // Green for Delivered
      'Sent': '#17a2b8',      // Blue for Sent
      'Processing': '#ffc107',// Yellow for Processing
      'Queue': '#fd7e14',     // Orange for Queue
      'Hold': '#6c757d'       // Gray for Hold
    };

    // Prepare the series data for each status and dynamically handle colors
    const seriesData = statusList.map(status => {
      return {
        name: status,
        data: Array.from({ length: 12 }, (_, monthIndex) => {
          // Get the count for each status by month
          const value = monthlyStatusCounts[monthIndex + 1][status] || 0;
          return value > 0 ? value : 0; // Show 0 if the value is not greater than 0
        })
      };
    });
    ...
    const barChartConfig = {
      ...
      plotOptions: {
        bar: {
          columnWidth: '15%',
          // Apply the background bar colors dynamically
          colors: {
            backgroundBarColors: statusList.map(status =>
              Array.from({ length: 12 }, (_, monthIndex) => {
                // Get the count for each status by month
                const value = monthlyStatusCounts[monthIndex + 1][status] || 0;
                return value > 0 ? statusColors[status] : '#d3d3d3'; // Apply color or gray
              })
            ).flat(), // Flatten the colors array for all statuses
      ...
      colors: statusList.map(status => statusColors[status]), // Use status colors for the series
      stroke: {
        show: true,
        colors: ['transparent']
      },
      ...
      series: seriesData, // Set the series dynamically
      xaxis: {
        categories: ['Jan', 'Feb', 'March', 'April', 'May', 'Jun', 'July', 'August', 'Sept', 'Oct', 'Nov', 'Dec'],
        axisBorder: {
          show: false
        },
        axisTicks: {
  ...
  const lineChartEl = document.querySelector('#lineChart'),
    lineChartConfig = {
      ...
      series: [
        {
          name: 'Transactions',
          data: last7DaysTransections.map(item => item.total_amount) // Extract total_amount
        }
      ],
      markers: {
        strokeWidth: 7,
      ...
      xaxis: {
        categories: last7DaysTransections.map(item => item.day), // Extract day names
        axisBorder: {
          show: false
        },
        axisTicks: {
  ...
  // Extract the series data (total_messages) and categories (days)
  var messageCounts = last7DaysMessages.map(item => item.total_messages);
  var messageDays = last7DaysMessages.map(item => item.day);
  ...
  const lineChartEl2 = document.querySelector('#lineChart2'),
    lineChart2Config = {
      ...
      series: [
        {
          name: 'Messages',
          data: messageCounts // Use dynamic data
        }
      ],
      markers: {
        strokeWidth: 7,
      ...
      xaxis: {
        categories: messageDays, // Use dynamic days
        axisBorder: {
          show: false
        },
        axisTicks: {
  ...
  if (typeof lineChartEl2 !== undefined && lineChartEl2 !== null) {
    const lineChart2 = new ApexCharts(lineChartEl2, lineChart2Config);
    lineChart2.render();
  }
  ...
  // Donut Chart
  // --------------------------------------------------------------------
  /*const labels = statusWiseMessagess.map(item => item.status);
  const series = statusWiseMessagess.map(item => item.percentage);
